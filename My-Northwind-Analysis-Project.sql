...
--  JOIN queries to combine customer, order, and product tables --
SELECT
    c.id AS customer_id,
    c.first_name,
    c.last_name,
...
SELECT
    o.order_date,
    (od.quantity * od.unit_price) AS sale_amount,
...
-- Grouping Revenue Based on month and Year --
SELECT
...
GROUP BY month
ORDER BY month;
-- Grouping Products based on Revenue --
 SELECT
    p.product_name,
    SUM(od.quantity * od.unit_price) AS revenue
FROM products p
...
SELECT
    ...
    CASE
        ...
        WHEN SUM(od.quantity * od.unit_price) >= 5000 THEN 'Medium Value'
        ELSE 'Low Value'
